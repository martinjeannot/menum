Ext.define('Ux.locale.Manager', {
    ...
    _ajaxDecoder : function(options, success, response) {
        var text = response.responseText;

        return Ext.decode(text);
    },

    _scriptDecoder : function(content) {
    ...
    /**
     * Retrieves localization file (JSON) via Ajax request. Configuration goes something like this :
    ...
    /**
     * Retrieves localization file (JS) via script tag injection, JSONP style. Configuration goes something like this :
     * Ux.locale.Manager.setConfig({
     *      language : ‘en’,
     *      tpl : ‘locales/{locale}.js’,
     *      type : ‘script’
     *  });
     *  Each localization file must use the callback Ux.locale.Manager.loadBundle to wrap the response.
     * @param callback
     */
    loadScriptTag : function(callback) {
        ...
        var language = me._language,
            path = me._tpl.replace('{locale}', language);

        Ext.Loader.loadScriptFile(path, callback, Ext.emptyFn, me, false);
    },

    loadBundle : function(bundle) {
        var me = this;

        me._locale = me._scriptDecoder(bundle);
        me._loaded = true;
    },
